                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <input type="hidden" name="type" value="parcelle">
                                    <select name="id_variete" id="">
                                        <option  checked>Types de variete</option>
                                        <?php for ($i=0; $i < count($variete); $i++) {  ?>
                                            <option value="<?php   echo $variete[$i]['id_variete'] ;   ?>"><?php   echo $variete[$i]['nom'] ;   ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <input type="text" class="form-control" id="nametext" name="surface"
                                        placeholder="Surface">
                                </div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php for ($i=0; $i < count($liste); $i++) { ?>
                                        <tr>
                                            <th scope="row"><?php  echo $liste[$i]['id_parcelle'] ; ?></th>
                                            <th scope="row"><?php  echo $liste[$i]['surface'] ; ?></th>
                                            <th scope="row"><?php  echo $liste[$i]['nom'] ; ?></th>
                                            <td>
                                                <div class="form-actions">
                                                    <button type="button" class="btn btn-info"><a href="action.php?type=parcelle&action=delete&id=<?php echo $liste[$i]['id_parcelle'] ; ?>">Delete </a></button>
                                                    <button type="button" class="btn btn-dark"><a href="action.php?type=parcelle&action=update&id=<?php echo $liste[$i]['id_parcelle'] ; ?>">Edit </a> </button>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php  }   ?>
